        ...
        if "SUBJECT_NAME" in df.columns and "OBJECT_NAME" in df.columns:
            ...
            if graph_data:
                ...
                if "SUBJECT_NAME" in combined_graph_df.columns and "OBJECT_NAME" in combined_graph_df.columns:
                    ...
                    st.write(f"### Top 5 Common Relationships from the Top 10 Ranked Model Scores (`{graph_choice}`, `{test_dataset}`)")
...
st.write(f"This app embeds the Verifai Project application directly into the Streamlit interface. You can search for "Can `{lomefloxacin}` be used in the treatment of `{Acute gonococcal cervicitis}` ?")
